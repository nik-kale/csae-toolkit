document.addEventListener('DOMContentLoaded', function () {
  ...
  loadButton.addEventListener('click', () => {
    ...
    chrome.devtools.inspectedWindow.eval(code, (result, isException) => {
      if (isException) {
        jsonEditorContainer.textContent = 'Failed to load storage data: ' + isException.value;
      } else {
        chrome.devtools.inspectedWindow.eval(
          'window.__devtools_storage_data;',
          (storageData, storageException) => {
            if (storageException) {
              jsonEditorContainer.textContent =
                'Failed to load storage data: ' + storageException.value;
            } else if (storageData) {
              // Syntax-highlighted output; prettyPrintJson escapes <, >, & before
              // building its <span> markup, so parsed values cannot inject HTML.
              const pre = document.createElement('pre');
              pre.className = 'json-container';
              pre.innerHTML = prettyPrintJson.toHtml(JSON.parse(storageData), {
                indent: 5,
                lineNumbers: false,
                linkUrls: true,
                linksNewTab: true,
              ...
              jsonEditorContainer.replaceChildren(pre);
            } else {
              jsonEditorContainer.textContent = 'Storage data is empty.';
            }
          }
        );
      }
  ...
  clearButton.addEventListener('click', () => {
    ...
    chrome.devtools.inspectedWindow.eval(code, (result, isException) => {
      if (isException) {
        jsonEditorContainer.textContent = 'Failed to clear storage data: ' + isException.value;
      } else {
        const pre = document.createElement('pre');
        pre.textContent = 'Storage cleared successfully.';
        jsonEditorContainer.replaceChildren(pre);
      }
    });
  });
